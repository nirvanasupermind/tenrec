...
var tenrec = new function () {
     ...
     */
    function Parser(evaluate) {
        if (!(this instanceof Parser)) {
            return new Parser(evaluate);
        } else {
    ...
    Parser.prototype.then = function (other,opt=false) {
        ...
        return Object.assign(new Parser(function (input) {
            var i = 0;
            while (i < input.length && parser1.parse(input.substring(0, i)) instanceof ParserError) {
                i++;
            }
            ...
            // console.log(i,a,b);
            var t1 = parser1.evaluate(a);
            if (t1 instanceof ParserError) return t1;

            var t2 = parser2.evaluate(b);
    ...
    Parser.prototype.map = function (f) {
        ...
        return new Parser(function (input) {
            var temp = parser1.evaluate(input);
